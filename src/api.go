	...
	if maintenanceMode {
		...
		clientAuth.Use(ClientAuthMiddleware())
		{
			...
			clientAuth.POST("/v1/internal/auto-annotate/:imageid",  func(c *gin.Context) {
				...
				var annotationsWithRefinements []datastructures.AnnotationsWithRefinements
				annotationsWithRefinements = append(annotationsWithRefinements, datastructures.AnnotationsWithRefinements{
																					Annotations: &annotations, 
																					Refinements: annotationsValidator.GetRefinements(), 
																					AutoGenerated: true,
																				})

				_, err = imageMonkeyDatabase.AddAnnotations(apiUser, imageId, annotationsWithRefinements)
				if err != nil {
		...
		router.POST("/v1/annotate/:imageid", func(c *gin.Context) {
			...
			annotationIds, err := imageMonkeyDatabase.AddAnnotations(apiUser, imageId, annotationsWithRefinements)
			if err != nil {
				c.JSON(500, gin.H{"error": "Couldn't add annotations - please try again later"})
				return
			}
