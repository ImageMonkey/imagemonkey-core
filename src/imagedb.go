...
func getAnnotatedImage(annotationId string, autoGenerated bool, revision int32) (AnnotatedImage, error) {
    ...
    if revision != -1 && annotationId != "" {
        q = `SELECT q1.key, l.name, COALESCE(pl.name, ''), q1.annotation_uuid, q.annotations as annotations, q1.num_of_valid, q1.num_of_invalid,
                                 q1.width, q1.height 
                                   FROM (
                                     ...
                                     q2.num_of_invalid as num_of_invalid, i.width as width, i.height as height
                                     FROM image i
                                     JOIN image_provider p ON i.image_provider_id = p.id
                                     JOIN (
                                   ...
                                   (
                                     ...
                                     COALESCE(d.image_annotation_id, image_annotation_revision_id) as annotation_id
                                     FROM annotation_data d 
                                     JOIN annotation_type t on d.annotation_type_id = t.id
                                     GROUP BY d.image_annotation_id, d.image_annotation_revision_id
                                   ...
                                   q1.num_of_valid, q1.num_of_invalid, q.annotations, q1.width, q1.height`
        ...
        q = fmt.Sprintf(`SELECT q1.key, l.name, COALESCE(pl.name, ''), q1.annotation_uuid, q.annotations as annotations, q1.num_of_valid, q1.num_of_invalid,
                                 q1.width, q1.height 
                                   FROM (
                                   ...
                                   LEFT JOIN label pl ON l.parent_id = pl.id
                                   GROUP BY q1.key, q1.annotation_uuid, l.name, pl.name, 
                                   q1.num_of_valid, q1.num_of_invalid, q.annotations, q1.width, q1.height`, q1)
    }
...
func getAnnotations(parseResult ParseResult) ([]AnnotatedImage, error) {
    ...
    q := fmt.Sprintf(`SELECT q1.key, l.name, COALESCE(pl.name, ''), q1.annotation_uuid, 
                             q.annotations as annotations, q1.num_of_valid, q1.num_of_invalid,
                                 q1.width, q1.height 
                                   FROM (
                                   ...
                                   LEFT JOIN label pl ON l.parent_id = pl.id
                                   GROUP BY q1.key, q1.annotation_uuid, l.name, pl.name, 
                                   q1.num_of_valid, q1.num_of_invalid, q.annotations, q1.width, q1.height`, parseResult.query)
