...
func _addImageValidationSources(imageSourceId int64, imageValidationIds []int64, tx pgx.Tx) error {
	for _, id := range imageValidationIds {
		_, err := tx.Exec(context.TODO(),
			"INSERT INTO image_validation_source(image_source_id, image_validation_id) VALUES($1, $2)", imageSourceId, id)
		if err != nil {
	...
	if len(validEntries) > 0 {
		rows1, err := tx.Query(context.TODO(),
			`UPDATE image_validation AS v 
                              ...
                              WHERE uuid = ANY($2) RETURNING id`,
			apiUser.ClientFingerprint, validEntries, stepSize)
		if err != nil {
			tx.Rollback(context.TODO())
...
func (p *ImageMonkeyDatabase) GetImageToValidate(validationId string, username string) (datastructures.ValidationImage, error) {
	...
	rows, err = p.db.Query(context.TODO(), q, queryParams...)
	...
	if !rows.Next() {
		...
		otherRows, err := p.db.Query(context.TODO(), q1, queryParams...)
...
func (p *ImageMonkeyDatabase) BlacklistForAnnotation(validationId string, apiUser datastructures.APIUser) error {
	_, err := p.db.Exec(context.TODO(),
		`INSERT INTO user_annotation_blacklist(image_validation_id, account_id)
...
func (p *ImageMonkeyDatabase) MarkValidationAsNotAnnotatable(validationId string) error {
	_, err := p.db.Exec(context.TODO(),
		`UPDATE image_validation SET num_of_not_annotatable = num_of_not_annotatable + 1 
...
func (p *ImageMonkeyDatabase) GetUnannotatedValidations(apiUser datastructures.APIUser, imageId string) ([]datastructures.UnannotatedValidation, error) {
	...
	if apiUser.Name == "" {
		rows, err = p.db.Query(context.TODO(), q, imageId)
	} else {
		rows, err = p.db.Query(context.TODO(), q, imageId, apiUser.Name)
	}
