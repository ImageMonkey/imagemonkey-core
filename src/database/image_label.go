...
func (p *ImageMonkeyDatabase) GetTrendingLabels() ([]datastructures.TrendingLabel, error) {
	...
	rows, err := p.db.Query(context.TODO(),
		`SELECT s.name, t.github_issue_id, t.closed, COALESCE(tb.state::text, ''), 
									...
									COALESCE(tb.plural, ''), COALESCE(tb.rename_to, ''), COUNT(s.id)
							 ...
							 GROUP BY s.name, t.github_issue_id, t.closed, tb.state, tb.job_url, tb.label_type, tb.branch_name,
							 tb.description, tb.plural, tb.rename_to`)
	if err != nil {
