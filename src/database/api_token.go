...
func (p *ImageMonkeyDatabase) GetApiTokens(username string) ([]datastructures.APIToken, error) {
	...
	rows, err := p.db.Query(context.TODO(),
		`SELECT token, issued_at, description, revoked 
                           FROM api_token a
...
func (p *ImageMonkeyDatabase) IsApiTokenRevoked(token string) (bool, error) {
	...
	rows, err := p.db.Query(context.TODO(), "SELECT revoked FROM api_token WHERE token = $1", token)
	if err != nil {
...
func (p *ImageMonkeyDatabase) GenerateApiToken(jwtSecret string, username string, description string) (datastructures.APIToken, error) {
	...
	_, err = p.db.Exec(context.TODO(),
		`INSERT INTO api_token(account_id, issued_at, description, revoked, token, expires_at)
