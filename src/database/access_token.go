...
func (p *ImageMonkeyDatabase) AddAccessToken(username string, accessToken string, expirationTime int64) error {
	...
	insertedId = 0
	err := p.db.QueryRow(context.TODO(), `INSERT INTO access_token(user_id, token, expiration_time)
...
func (p *ImageMonkeyDatabase) RemoveAccessToken(accessToken string) error {
	_, err := p.db.Exec(context.TODO(), `DELETE FROM access_token WHERE token = $1`, accessToken)
	if err != nil {
...
func (p *ImageMonkeyDatabase) AccessTokenExists(accessToken string) bool {
	...
	numOfAccessTokens = 0
	err := p.db.QueryRow(context.TODO(), "SELECT count(*) FROM access_token WHERE token = $1", accessToken).Scan(&numOfAccessTokens)
	if err != nil {
		log.Debug("[Add Access Token] Couldn't add access token: ", err.Error())
		raven.CaptureError(err, nil)
		return false
