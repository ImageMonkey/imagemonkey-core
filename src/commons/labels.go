...
func GenerateLabelEntryAndPersistToFile(trendingLabel datastructures.TrendingLabelBotTask, dir string) (string, error) {
	...
	if(trendingLabel.Parent == "") {
		...
		fileName = labelEntry.Uuid
	} else {
		labelName = trendingLabel.Parent
		...
		parentLabelFileName := filepath.Base(parentLabelFilePath)
		fileName = strings.TrimSuffix(parentLabelFileName, filepath.Ext(parentLabelFileName))
		...
		if labelEntry.LabelMapEntries == nil {
			labelEntry.LabelMapEntries = make(map[string]datastructures.LabelMapEntry)
		}
		labelEntry.LabelMapEntries[trendingLabel.RenameTo] = subLabelEntry
	}

	autoGeneratedLabelsWriter := NewLabelsWriter(dir + fileName + ".json")
	err = autoGeneratedLabelsWriter.Add(labelName, labelEntry)
	if err != nil {
