...
func addAccessors(tx *sql.Tx, label string, val LabelMapEntry) {
	for _, accessor := range val.Accessors {
		...
		for _, quizEntry := range val.Quiz {
			for _, quizAnswer := range quizEntry.Answers {
				_,err := tx.Exec(`UPDATE label AS l
									  SET uuid = $1
									  FROM label pl
									  WHERE l.parent_id = pl.id AND l.name = $2 AND pl.name = $3`, quizAnswer.Uuid, quizAnswer.Name, label)
				if err != nil {
					tx.Rollback()
					log.Fatal(err.Error())
					panic(err)
...
func main(){
	...
	for k := range labelMap {
		...
		if numOfLabels == 0 {
			...
			_,err := tx.Exec(`UPDATE label AS l
							  SET uuid = $1
							  WHERE l.name = $2 AND l.parent_id is null`, val.Uuid, k)
			if err != nil {
				tx.Rollback()
				log.Fatal(err)
				panic(err)
		...
		if len(val.LabelMapEntries) != 0 {
			for sublabel, subval := range val.LabelMapEntries {
				...
				if numOfLabels == 0 {
					...
					log.Info("update label ", sublabel, " (parent: ", k, " ) and set uuid to ", subval.Uuid)
					_,err := tx.Exec(`UPDATE label AS l
									  SET uuid = $1
									  FROM label pl
									  WHERE l.parent_id = pl.id AND l.name = $2 AND pl.name = $3`, subval.Uuid, sublabel, k)
					if err != nil {
						tx.Rollback()
						log.Fatal(err)
						panic(err)
