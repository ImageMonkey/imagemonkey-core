...
func (p *LabelsRepository) AddLabelAndPushToRepo(trendingLabel TrendingLabel) (string, error) {	
	...
	log.Info(p.autoGeneratedLabelsWriter.GetFilename())
	if err != nil {
...
func getTrendingLabels() ([]TrendingLabel, error) {
	...
	rows, err := db.Query(`SELECT s.name, b.id, b.state, COALESCE(b.branch_name, '')
	...
	for rows.Next() {
		...
		err = rows.Scan(&trendingLabel.Name, &trendingLabel.BotTaskId, &trendingLabel.State, &trendingLabel.BranchName)
		if err != nil {
			return trendingLabels, err
		}
